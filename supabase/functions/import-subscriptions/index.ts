...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await supabaseClient.from('subscriptions').delete().neq('id', 'none');

    // Add all subscriptions to the database
    const { data, error } = await supabaseClient.from('subscriptions').insert(subscriptionsData);

    if (error) throw error;

    return new Response(
      JSON.stringify({ success: true, message: 'Subscriptions imported successfully' }),
      { 
        headers: { 
          ...corsHeaders,
